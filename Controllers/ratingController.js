...
const rides=await RideRequest.find({acceptedBy:req.user._id,status:"completed"})
const totalRides=rides.length
  res.status(200).json({
    status: 200,
    success: true,
    message: '',
    data: {ratings,totalRides,averageRating:req.user.averageRating,totalReviews:req.user.totalReviews},
  });
});
...
exports.store = catchAsync(async (req, res, next) => {
  ...
  const rating = await Ratings.findOneAndUpdate({	from,to,requestId},req.body,{upsert:true});
...
exports.update = catchAsync(async (req, res, next) => {
    const ratings = await Ratings.findByIdAndUpdate(req.params.id, {$set: JSON.parse(JSON.stringify(req.body))}, { new: true });
...
exports.delete = catchAsync(async (req, res, next) => {
  let ratings =  await Ratings.findOne(req.params.id ? { _id: req.params.id } : JSON.parse(decodeURIComponent(req.query)))
