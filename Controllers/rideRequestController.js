...
exports.index = catchAsync(async (req, res, next) => {
  const rideRequests = await RideRequest.find({$and:[{$or:[{status:"completed"},{status:"pending"}]},{$or:[{requestedBy:req.user._id},{acceptedBy:req.user._id}]}]} )
...
exports.store = (async (req, res, next) => {
  ...
  const rideRequest = await RideRequest.create({...JSON.parse(JSON.stringify(req.body)), requestedBy: req.user._id});
...
setTimeout(async function() {
  ...
  if(rideRequest2.status == "pending") await RideRequest.findByIdAndDelete(rideRequest._id)
...
const f=async()=>{
  ...
  console.log("userrrrrrrrrr",user.location.coordinates[0],process.env.RIDE_RADIUS)
const users=await User.find({online:true,location: { $geoWithin: { $centerSphere: [[user.location.coordinates[0],user.location.coordinates[1]], 4/6378.1] } }})
...
const request=await RideRequest.findById(req.body.requestId)
 ...
 await RideRequest.findByIdAndUpdate(request._id,{$set:{status:"accepted",acceptedBy:req.user._id}})
 const updatedRequest=await RideRequest.findById(req.body.requestId).populate('acceptedBy')
 res.status(200).json({
  status: 200,
  success: true,
  message: 'accepted',
  ...
  res.to(request.requestedBy.toString()).status(200).json({
    status: 200,
    success: true,
...
const user= await User.findById(req.user._id)
    const requests=await RideRequest.find({status:"pending",startLocation: { $geoWithin: { $centerSphere: [[user.location.coordinates[0],user.location.coordinates[1]], 4/6378.1] } }}).populate('requestedBy')
...
//
  ...
  const user= await User.findById(req.user._id)
      const requests=await RideRequest.find({status:"pending",startLocation: { $geoWithin: { $centerSphere: [[user.location.coordinates[0],user.location.coordinates[1]], 4/6378.1] } },endLocation: { $geoWithin: { $centerSphere: [[req.body.destination.coordinates[0],req.body.destination.coordinates[1]], 4/6378.1] } }}).populate('requestedBy')
...
const users=await User.findByIdAndUpdate(req.body.user._id,{location: req.body.location})
 
res.to(req.body.user._id.toString()).status(200).json({
    status: 200,
    success: true,
    message: 'updated',
...
if(req.body.to){
  res.to(req.body.to.toString()).status(200).json({
    status: 200,
    success: true,
    message: 'updated',
    data: {location: req.body.location},
...
exports.update = catchAsync(async (req, res, next) => {
    const rideRequest = await RideRequest.findByIdAndUpdate(req.params.id, {$set: JSON.parse(JSON.stringify(req.body))}, { new: true });
...
exports.delete = catchAsync(async (req, res, next) => {
  let rideRequest =  await RideRequest.findOne(req.params.id ? { _id: req.params.id } : JSON.parse(decodeURIComponent(req.query)))
...
exports.myCurrentStatus = catchAsync(async (req, res, next) => {
  let request =  await RideRequest.findOne({$and:[{status:{$ne:"completed"}},{status:{$ne:"canceled"}},{acceptedBy:req.user._id}]})
...
exports.cancelRequest = (async (req, res, next) => {
  ...
  //   });
  //  }
   await RideRequest.findByIdAndUpdate(request._id,{$set:{status:"canceled",canceledBy:req.user._id,reason:req.body.reason}})
   const updatedRequest=await RideRequest.findById(req.body.requestId).populate('acceptedBy').populate('requestedBy')
   res.status(200).json({
    status: 200,
    success: true,
    message: 'canceled',
    ...
    res.all(request.acceptedBy.toString()).status(200).json({
      status: 200,
      success: true,
    ...
    // console.log("customerrr>",request.requestedBy.toString())
    res.all(request.requestedBy.toString()).status(200).json({
      status: 200,
      success: true,
    ...
    exports.startRide = (async (req, res, next) => {
      ...
      const request=await RideRequest.findById(req.body.requestId)
       ...
       res.status(200).json({
        ...
        res.to(request.acceptedBy.toString()).status(200).json({
          status: 200,
          success: true,
          message: 'started',
          data: {request:updatedRequest},
           ...
           res.status(200).json({
            ...
            exports.rideEnd = (async (req, res, next) => {
              ...
              const request=await RideRequest.findById(req.body.requestId)
               ...
               await RideRequest.findByIdAndUpdate(request._id,{$set:{rideEnd:true,status:"completed"}})
               const updatedRequest=await RideRequest.findById(req.body.requestId).populate('acceptedBy').populate('requestedBy')
               res.status(200).json({
                status: 200,
                success: true,
                ...
                exports.paymentReceived = (async (req, res, next) => {
                  ...
                  res.all(req.body.to.toString()).status(200).json({
                    status: 200,
                    success: true,
